...
class EnhancedGenerator(nn.Module):
    def __init__(self, CHANNEL):
        ...
        # Warstwy enkodera
        self.encoder1 = self.encoder_block(CHANNEL, 128)  # 64x64 -> 32x32
        self.encoder2 = self.encoder_block(128, 256)  # 16x16
        self.encoder3 = self.encoder_block(256, 512)  # 8x8
        self.encoder4 = self.encoder_block(512, 1024)  # 4x4

        self.bottleneck = self.bottleneck_block(1024, 2048)  # 2x2
        ...
        self.decoder1 = self.decoder_block(256, 128)

        self.skip_conv4 = nn.Conv2d(2048, 1024, kernel_size=1)
        self.skip_conv3 = nn.Conv2d(1024, 512, kernel_size=1)
        self.skip_conv2 = nn.Conv2d(512, 256, kernel_size=1)
        self.skip_conv1 = nn.Conv2d(256, 128, kernel_size=1)

        self.final_conv = nn.ConvTranspose2d(128, CHANNEL, kernel_size=4, stride=2, padding=1)
...
class EnhancedDiscriminator(nn.Module):
    def __init__(self, CHANNEL):
        ...
        self.conv1 = self.discriminator_block(CHANNEL, 128, batch_norm=False)  # 32x32
        self.conv2 = self.discriminator_block(128, 256)  # 16x16
        self.conv3 = self.discriminator_block(256, 512)  # 8x8
        self.conv4 = self.discriminator_block(512, 1024)  # 4x4
        self.conv5 = self.discriminator_block(1024, 2048)  # 2x2
